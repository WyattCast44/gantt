...
import { CLASSIFICATIONS, type BaseClassificationValue, type Comment } from '@/types';
...
/**
 * A commentable-agnostic comment thread. Both the document and task detail
 * pages drive this with their own route URLs, so the thread UI lives in one
 * place. Editors+ can post; authors edit/delete their own; owners/admins delete
 * any (abilities arrive per comment in the `can` block from CommentResource).
 */
export type CommentsThreadProps = {
    comments: Comment[];
    /** Whether the current viewer may post (editors+). */
    canComment: boolean;
    /** Classification options capped at the project baseline. */
    options: typeof CLASSIFICATIONS;
    storeUrl: string;
    buildUpdateUrl: (commentId: number) => string;
    buildDestroyUrl: (commentId: number) => string;
};

export default function CommentsThread({ comments, canComment, options, storeUrl, buildUpdateUrl, buildDestroyUrl }: CommentsThreadProps) {
    return (
        <div className="flex flex-col gap-6">
            {canComment && <CommentComposer options={options} storeUrl={storeUrl} />}
                ...
                <ul className="flex flex-col gap-3">
                    {comments.map((comment) => (
                        <CommentRow
                            ...
                            comment={comment}
                            options={options}
                            updateUrl={buildUpdateUrl(comment.id)}
                            destroyUrl={buildDestroyUrl(comment.id)}
                        />
                    ))}
                </ul>
            )}
...
function CommentComposer({ options, storeUrl }: { options: typeof CLASSIFICATIONS; storeUrl: string }) {
                        ...
                        {(comment.can.update || comment.can.delete) && !editing && (
                            <DropdownMenu>
                                <DropdownMenuTrigger
                                    ...
                                    className="p-1 text-slate-500 hover:text-slate-700 dark:text-neutral-400 dark:hover:text-neutral-200"
                                >
                        ...
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            <div className="sm:w-56">
                                <Select
                                    ...
                                    onChange={(event) => editForm.setData('base_classification', event.target.value as BaseClassificationValue)}
                                >
                                    {options.map((option) => (
                                        <option key={option.value} value={option.value}>
                                            {option.label}
